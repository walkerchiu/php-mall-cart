    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            ...
            if ( config('wk-mall-cart.onoff.mall-shelf') && !empty(config('wk-core.class.mall-shelf.stock')) ) {
                if ( isset($data['stock_id']) && isset($data['nums']) && is_array($data['stock_id']) && is_array($data['nums']) ) {
                    $map = [];
                    ...
                    if ( isset($data['binding']) && is_array($data['binding']) ) {
                        foreach ($data['binding'] as $binding_array) {
                            if (empty($binding_array) || !is_iterable($binding_array)) continue;
