...
class ItemRepository extends Repository
{
    ...
    {
        $this->entity = App::make(config('wk-core.class.mall-cart.item'));
    }
     ...
     * @return Array
     */
    public function showByChannelId(Int $id, $code, $user_id = null)
    {
        $list = [];
        if (is_array($code)) {
        ...
        $items = $this->entity->with('stock', 'stock.product', 'stock.catalog')
                              ->where('channel_id', $id)
     ...
     * @return Int
     */
    public function countNums(Int $channel_id, Int $user_id, Int $stock_id)
    {
     ...
     * @return Entity
     */
    public function push(String $code, Int $channel_id, Int $user_id, Int $stock_id, $nums = 1, $binding = null, $options = null)
    {
        $record = $this->where('channel_id', '=', $channel_id)
                       ->where('user_id', '=', $user_id)
                       ->where('stock_id', '=', $stock_id)
     ...
     * @return Entity
     */
    public function pop(String $code, Int $channel_id, Int $user_id, Int $stock_id, $nums = -1)
    {
        $record = $this->where('channel_id', '=', $channel_id)
                       ->where('user_id', '=', $user_id)
                       ->where('stock_id', '=', $stock_id)
        ...
        if (!empty($record) && ($record->nums+$nums) > 0) {
            $record->nums += $nums;
            $record->save();
     ...
     * @return Entity
     */
    public function updateAll(String $code, Int $channel_id, Int $user_id, Array $stocks, Array $nums)
    {
        if (count($stocks) != count($nums))
            throw new NotExpectedEntityException($stocks);
        ...
        foreach ($stocks as $key=>$stock_id) {
            ...
            if (empty($record)) {
                throw new NotFoundEntityException($record);
            } else {
     ...
     * @return Boolean
     */
    public function remove(Int $channel_id, Int $user_id, Array $stock_id)
    {
